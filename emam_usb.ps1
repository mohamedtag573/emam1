...
# مسار الـ scripts جنب الملف على الفلاشة
...
# ================================================================
#  RUN OPTION من الفلاشة مباشرة
# ================================================================
function Run-Option($num) {
    $path = Join-Path $ScriptDir "scripts\$num"
    if (Test-Path $path) {
        & powershell.exe -ExecutionPolicy Bypass -File "$path"
    } else {
        Write-Host ""
        Write-Host "    [!] Script not found on USB: scripts\$num" -ForegroundColor Red
        Write-Host ""
        Start-Sleep 2
    }
}
